...
function(input, output, session) {
  ...
  observeEvent(c(input$bites_rate, input$T_HM, input$T_MH, input$infectious_period, input$mu_M), {
  ...
  output$humanPlot <- renderPlotly({
    ...
    plot_ly(human_df, x = ~time, y = ~Count, color = ~Compartment, type = 'scatter', mode = 'lines') %>%
      layout(
  ...
  output$mosquitoPlot <- renderPlotly({
    ...
    plot_ly(mosquito_df, x = ~time, y = ~Count, color = ~Compartment, type = 'scatter', mode = 'lines') %>%
      layout(
  ...
  observeEvent(c(input$bites_rate_E, input$T_HM_E, input$T_MH_E, input$infectious_period_E, 
                 input$mu_M_E, input$exposed_period_H, input$exposed_period_M), {
  ...
  dengue_with_E <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      Recovered_human_change <- Infectious_human / infectious_period
      Susceptible_mosquito_change <- mu_M * Total_mosquito_population - (force_of_infection_on_mosquito + mu_M) * Susceptible_mosquito
      Exposed_mosquito_change <- force_of_infection_on_mosquito * Susceptible_mosquito - Exposed_mosquito / exposed_period_M
